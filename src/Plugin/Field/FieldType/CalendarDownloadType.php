...
class CalendarDownloadType extends FieldItemBase {
   ...
   * {@inheritdoc}
   */
  public function __construct(DataDefinitionInterface $definition, $name = NULL, TypedDataInterface $parent = NULL, $tokenService) {
  ...
  public static function schema(FieldStorageDefinitionInterface $fieldDefinition) {
    $targetTypeInfo = \Drupal::entityManager()->getDefinition('file');
    $schema = [
      'columns' => [
        'summary' => [
        ...
        'fileref' => [
          ...
          'length' => $targetTypeInfo->getBundleOf() ? EntityTypeInterface::BUNDLE_MAX_LENGTH : 255,
        ],
      ],
    ];
   ...
   */
  public function checkWriteableDirectory(&$element, FormStateInterface $formState, &$completeForm) {
    ...
    if (!file_prepare_directory($uploadLocation, FILE_CREATE_DIRECTORY)) {
      $formState->setError($element, $this->t('Cannot create folder for ics files [@upload_location]', ['@upload_location' => $uploadLocation]));
    }
  }
